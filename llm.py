...
def generate_search_keywords(news_text: str) -> str:
    ...
    กฎ (ปฏิบัติตามอย่างเคร่งครัด): 
    1. สกัดเฉพาะ "ชื่อหน่วยงาน/บุคคล", "สถานที่", หรือ "เหตุการณ์หลัก" ที่มีอยู่จริงในเนื้อหา
    2. ห้ามแปลเป็นภาษาอังกฤษ ตอบเป็นภาษาไทยเท่านั้น 
    3. ห้ามมีสัญลักษณ์พิเศษใดๆ (เช่น "" - * # : []) ให้เว้นวรรคระหว่างคำเท่านั้น
    4. ตอบแค่กลุ่มคำ 3-6 คำ
    """
    for attempt in range(3):
        try:
            response = requests.post(
