...
def classify_content(news_text: str) -> tuple:
    ...
    # 🛡️ กฎเหล็ก (Heuristic Bypass): ถ้าเจอตัวเลข สถิติ เปอร์เซ็นต์ บังคับผ่าน 100% แก้อาการ AI มักง่ายปัดตกข่าวชัชชาติ
    if re.search(r'\d{1,3}(,\d{3})+|\d+(\.\d+)?\s?(%|เปอร์เซ็นต์|ล้าน|แสน|หมื่น|เสียง|คะแนน)', news_text):
        return "PROCEED", "ตรวจพบสถิติ ตัวเลข หรือข้อมูลเชิงปริมาณ (Heuristic Rule) จึงส่งเข้าตรวจสอบความจริงทันที"
